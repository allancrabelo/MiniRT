...
/*   Updated: 2025/12/13 17:33:10 by aaugusto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	err_message_parsing(int code)
{
	if (code == ERR_USAGE)
		...
		ft_putstr_fd("Extension error: scene file must have \
a .rt extension\n", 2);
		...
		ft_putstr_fd("File error: scene file is empty or contains \
no valid elements\n", 2);
	else if (code == ERR_INVALID_ELEMENT)
		ft_putstr_fd("Parsing error: invalid element \
identifier in scene file\n", 2);
		...
		ft_putstr_fd("Parsing error: duplicate parameter \
found in scene file\n", 2);
...
/**
 * @brief Prints scene element configuration error messages (part 1).
 * 
 * Handles error messages for basic element parameters including
 * resolution, ambient light, camera, and coordinate/orientation errors.
 * 
 * @param code Error code to display message for.
 */
...
light specified\n", 2);
		...
		err_message_elements_ranges(code);
}

/**
 * @brief prints MLX graphics library error messages.
 * 
 * @param code Error code to display message for.
 */
...
/**
 * @brief Prints data type error messages.
 * 
 * @param code Error code to display message for.
 */
...
/**
 * @brief Main error message dispatcher.
 * 
 * Formats error output with color coding and routes to specific
 * error message functions based on error code range.
 * 
